...
24. 「ToDoリストから消してもカレンダーには過去の記録として残す」論理削除(ソフトデリート)
    ...
    #### ⚠️ このSQLをSupabaseのSQL Editorで実行してから、コードをデプロイ(push)すること

    > **重要**: 下記SQLを実行する前にこの機能のコードを本番にデプロイすると、`deleted_at`列が
    > 存在しないためToDo取得クエリが失敗し、アプリ全体が表示できなくなります。そのため、
    > この機能のコミットは**pushせずローカルに留めてあります**。以下のSQLを実行したあとに
    > `git push` してデプロイしてください(または「実行した」と伝えてもらえれば私がpush・
    > 動作確認します)。ローカルの開発サーバーも同じ本番Supabaseを見ているため、SQL実行前は
    > ローカルでも同様に動きません。
...
- **【要対応】項目24(論理削除)のSQLをSupabaseで実行 → `git push` でデプロイ →
  動作確認**。上記「⚠️」の手順を参照。SQL未実行の状態でpushしないこと。
